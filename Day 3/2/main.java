...
class main {
    public static void main(String[] args) throws Exception{   
        ...
        fabric = findOverlap(claims, fabric);
        claims = findOverlap2(claims, fabric);

        for (int i = 0; i < claims.size(); i++){
            if (claims.get(i).overlapCount == 0){
                System.out.println(claims.get(i).overlapCount);                
    ...
    public static ArrayList<Claim> findOverlap2(ArrayList<Claim> claims, int[][] fabric){
        Claim claim;
        int overlapCount = 0;
        for (int claimIndex = 0; claimIndex < claims.size(); claimIndex++){   
            ...
            claim.overlapCount = overlapCount;
            claims.set(claimIndex, claim);
        }
        return claims;
    }
...
class Claim {
    public int id, marginLeft, marginTop, width, height, overlapCount;
